...
static void draw_present (struct draw *data)
{
  ...
  for (i = 0; i < 864; i++) {
    ...
    rgb += pitch * i;
    source = data->pixels;
    source += 1088 * i * sizeof (uint32_t);
    memcpy (rgb, source, 1088 * sizeof (uint32_t));
  }
...
static void motion (SDL_MouseMotionEvent *ev)
{
  mouse_motion ((int)ev->xrel, (int)ev->yrel);
}

static void button (SDL_MouseButtonEvent *ev)
{
  switch (ev->button)
    {
    case SDL_BUTTON_LEFT:
      mouse_button_left (ev->state == SDL_PRESSED);
      break;
    case SDL_BUTTON_MIDDLE:
      mouse_button_middle (ev->state == SDL_PRESSED);
      break;
    case SDL_BUTTON_RIGHT:
      mouse_button_right (ev->state == SDL_PRESSED);
      break;
    }
}
